...
void* proc(void * arg) {
    ...
    cout << "\nStarted thread " << args->sym << " with id: " << pthread_self();
       ...
       ts.tv_sec +=1;
        ...
        int re_value = pthread_mutex_timedlock(&mutexID,&ts);
        if(re_value !=0)
        {
        cout <<"\nFailed to lock mutex with error:" << strerror(re_value);
        sleep(1);
        continue;
        }  
        for( int i = 0;i < 5;i++)
        ...
        if(re_value != 0)
        {
        cout <<"\nFailed to unlock mutex with error: " << strerror(re_value);
        sleep(1);
        exit(EXIT_FAILURE);
        }
        sleep(1);       
...
int main(){
    ...
    if(re_value !=0){
    cout <<"\nError function pthread_mutex_init: " << strerror(errno);
    exit(EXIT_FAILURE);
    }
