...
void * write_pipe(void * arg){
     ...
     cout <<"\nEnd write with thread id: " << pthread_self() << endl;
...
void * read_pipe(void * arg){
     ...
     cout <<"\nEnd read with thread id: " << pthread_self() << endl;
     void * return_value = NULL;
     return return_value;
}
int main()
{
    ...
    cout <<"\nError function pipe2 with error: " << strerror(errno);
    sleep(1);
    exit(EXIT_FAILURE);
    }
    if(pthread_create(&id1,NULL,write_pipe,(void *)(&params_1)) != 0){
    cout <<"\nError function pthread_create for 1-th thread: " << strerror(errno);
    sleep(3);
    exit(EXIT_FAILURE);
    }
    if(pthread_create(&id2,NULL,read_pipe,(void*)(&params_2)) != 0){
